...
legend("topleft", legend = countries$countryname, 
  ...
  ncol = 2, bty = "n")
dev.print(png, filename = "Results/1_TSlength.png", 
  units = "in", res = 100)
...
x11()
boxplot(tot_spec[,spec_inds], border = spec_pal, lwd = 2, names = spec_names, col = "white")
dev.print(png, filename = "Results/1_SpecBoxplot.png", 
  units = "in", res = 100)
...
agg_spec <- aggregate(tot_spec[,spec_inds], 
  ...
  mean)
agg_pm25 <- aggregate(total ~ country + year, data = tot_spec, mean)
...
for (i in seq_len(nrow(countries))){
  ...
  lines(bp, country_pm25$total, type = "b", lwd = 2, 
    col = "black", xpd = T)
}
plot.new()
legend("topleft", spec_names, fill = spec_pal, bty = "n", ncol = 2,
  cex = .8, xpd = T, border = NA)
dev.print(png, filename = "Results/1_SpecCountries.png", 
  units = "in", res = 100)
...
x11()
par(mar = c(5, 4, 4, 7) + .1)
plot(as.numeric(tot_spec$year), (tot_spec$total - spec_sum) / tot_spec$total, 
