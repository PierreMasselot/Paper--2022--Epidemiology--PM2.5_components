...
plot(beta_spec, -seq_along(beta_spec), 
  xlab = "Meta-regression coefficient", ylab = "",
  axes = F, xlim = range(c(lo_spec, up_spec)))
segments(lo_spec, -seq_along(beta_spec), up_spec,
  -seq_along(beta_spec), col = "darkgrey", lwd = 2)
points(beta_spec, -seq_along(beta_spec), 
   col = spec_pal, cex = ifelse(sig_spec, 1.5, 1), 
   pch = ifelse(sig_spec, 15, 16))
axis(1)
axis(2, at = -seq_along(beta_spec), labels = spec_names, 
